  ...
  return ( 
    <main className="max-w-2xl mx-auto p-10 bg-white shadow-xl rounded-3xl mt-10"> 
      ...
      <form action={editarPratoComId} className="flex flex-col gap-4"> 
        ...
        <textarea name="descricao" defaultValue={prato.descricao} className="p-3 border 
rounded-xl h-32" /> 
         
        <select name="categoria" defaultValue={prato.categoria} className="p-3 border 
rounded-xl bg-white"> 
        ...
        <input name="preco" type="number" step="0.01" defaultValue={prato.preco} 
className="p-3 border rounded-xl" /> 
        <input name="imagem" type="url" defaultValue={prato.imagem} className="p-3 
border rounded-xl" /> 
 
        <label className="flex items-center gap-2 p-2"> 
          <input name="destacado" type="checkbox" defaultChecked={prato.destacado} 
