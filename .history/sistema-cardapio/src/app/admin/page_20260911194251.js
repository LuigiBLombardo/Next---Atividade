...
export default async function PaginaAdmin({ searchParams }) { 
  ...
  const resposta = await fetch('https://api-restaurante-5iqb.onrender.com/api/produtos’, { next: { revalidate: 0 } // Admin precisa de dados sempre frescos 
  }); 
  ...
  return ( 
    <main className="p-10 max-w-6xl mx-auto"> 
      <div className="flex justify-between items-center mb-8"> 
        ...
        <Link href="/admin/novo-prato" className="bg-green-600 text-white px-6 py-2 
rounded-lg font-bold hover:bg-green-700 transition-colors"> 
          + Novo Prato 
        </Link> 
      </div> 
        ...
        <table className="w-full text-left"> 
          ...
          <tbody> 
            {produtosFiltrados.map((prato) => ( 
              <tr key={prato.id} className="border-b last:border-none hover:bg-gray-50 
transition-colors"> 
